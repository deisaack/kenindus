...
class AfricasTalkingGateway:
    ...
    def sendMessage(self, to_, message_, from_=None, bulkSMSMode_=1, enqueue_=0, keyword_=None, linkId_=None,
        ...
        if self.responseCode == self.HTTP_RESPONSE_CREATED:
            decoded = json.loads(str_response)
    ...
    def sendRequest(self, urlString, data_=None):
        try:
            if data_ is not None:
                data = urllib.parse.urlencode(data_)
                data = data.encode('utf-8')
                request = urllib.request.Request(urlString, data, headers=self.headers)
            else:
                request = urllib.request.Request(urlString, headers=self.headers)
